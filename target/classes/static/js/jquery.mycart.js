  ...
  var loadMyCartEvent = function (targetSelector) {
    ...
    if (!$("#" + idCartModal).length) {
      $('body').append(
        ...
        '<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true" onclick="refreshVal()">&times;</span></button>' +
        ...
        '<button type="button" class="btn btn-default" data-dismiss="modal" onclick="refreshVal()">Close</button>' +
        '<button type="button" class="btn btn-primary ' + classCheckoutCart + '">Checkout & Buy</button>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>'
    ...
    */
	...
	var allot = parseInt($("#allotProd").val());
	$("#allotProd").val(allot-1);
	if($("#allotProd").val()>=0){
      var $target = $(this);
      ...
      $cartBadge.text(ProductManager.getTotalQuantity());
	  //alert(ProductManager.getTotalQuantity());

      options.afterAddOnCart(ProductManager.getAllProducts(), ProductManager.getTotalPrice(), ProductManager.getTotalQuantity());
	}else{
		alert("Limit exceed or unit not available");
		$("#allotProd").val(0);
	   }
    });
